...
import { pathologyTableColumns } from "@/constants/pathologyTableColumns";
...
const Pathology = () => {
  ...
  const fetchPathologyTableData = async (pageNo, searchValue) => {
    setLoading(() => true);
    ...
    try {
      const casesList = await getAllCases(
        `?type=PATHOLOGY&size=${page.size}&page=${currentPage}${searched}${sorted}`
      );
      ...
      const formattedRows = casesList?.content?.map((item) => {
        return {
          ...
          status: item.status,
          slideType: item.slides?.[0]?.organ || "--",
          slideNumber: item?.slides?.length || 0,
          date: item.date,
        };
      });
  ...
  useEffect(() => {
    fetchPathologyTableData(page.current ?? 0, search);
  }, [sort, reset]);
  ...
  const handleNavigate = (record) => {
    navigate(`/pathology/scan-details/${record.id}/case-details`);
  };

  return (
    <DashboardTemplate heading="Pathology">
      {caseDrawer && (
        <CaseDetailDrawer
          open={caseDrawer}
          caseType="PATHOLOGY"
          onClose={hideCaseDrawer}
          redirectTo="/pathology/scan-details"
      ...
      <section className="admin-table-fields-container">
        <h4 className="tabs-heading">List of Pathology</h4>
      ...
      <Table
        columns={pathologyTableColumns(handleNavigate, handleDateSort, sort)}
        data={tableData}
        loading={loading}
        page={page}
