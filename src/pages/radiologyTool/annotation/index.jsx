...
import {
  annotationCards,
  radiologistTableOptions,
  radiologistTableColumns,
} from "@/constants/radiologistToolColumns";
import {
  getAllCases,
  fetchCaseStatistics,
  assignCaseToMe,
...
const Annotation = () => {
  ...
  const navigateToCase = (record) => {
    navigate(`/radiology-annotation-tool/${record.id}/case-details`, {
  ...
  const fetchCardsData = async () => {
    try {
      const data = await fetchCaseStatistics("RADIOLOGY");
      setCaseStats(() => data);
    } catch (err) {
      console.log(err);
    }
  ...
  const fetchRadiologyTableData = async (pageNo, searchValue, appliedFilters = {}) => {
    setLoading(true);
    ...
    try {
      const casesList = await getAllCases(
        `?type=RADIOLOGY&status=${currentType}&size=${page.size}&page=${currentPage}${searched}${sorted}${filterQuery}`
      );
  ...
  const handleApplyFilters = (filterData) => {
    ...
    if (!isEmptyFilterData) {
      ...
      fetchRadiologyTableData(0, undefined, updatedFilters);
    }
  };

  const handleResetFilters = () => {
    fetchRadiologyTableData(page.current, search, null);
  };
  ...
  useEffect(() => {
    fetchRadiologyTableData(page.current, search);
  ...
  const options = radiologistTableOptions(role)?.filter((item) => {
    if (item) {
      return item;
    }
  });
  ...
  return (
    <DashboardTemplate heading="Radiology Tool">
      ...
      <Table
        columns={radiologistTableColumns(
          navigateToCase,
          handleDateSort,
          sort,
          type,
