...
const Editor = (props) => {
  ...
  const fetchAnnotationTypeList = async () => {
    ...
    try {
      const response = await getAnnotationTypeList("PATHOLOGY");
      ...
      const responsegetGradingList = await getGradingList(organName);
      const responseGradingList = responsegetGradingList?.data?.length
        ? responsegetGradingList.data.map((item) => ({
            id: item.id, 
            name: item.name,
            organName: item.organName, 
          }))
      ...
      const responsegetSubTypeList = await getSubTypeList(organName);
      const responseSubTypeList = responsegetSubTypeList?.data?.length
        ? responsegetSubTypeList.data.map((item) => ({
            id: item.id, 
            name: item.name,
            organName: item.organName, 
          }))
      ...
      setAnnotationTypeList((st) => ({
        ...
        gradingList: gradingList.concat(responseGradingList),
        subTypeList: subTypeList.concat(responseSubTypeList),
      }));
    } catch (err) {
      toast.error(err);
    } finally {
  ...
  const handleCreateAnnotation = async () => {
    ...
    const subtype = {
      iid: null,
      name: (formValues.subType === "NEW_SUB_TYPE"
        ? formValues.newSubType
        : formValues.subType
      ).toUpperCase(),
      organName: slides[0]?.organ || "",
    };
    ...
    const grading = {
      id: null,
      name: (formValues.grading === "NEW_GRADING"
        ? formValues.newGradingType
        : formValues.grading
      ).toUpperCase(),
      diseaseSpectrumName: diseaseSpectrum.name, 
      organName: slides[0]?.organ || "",
    };
    ...
    if (!isEdit) {
      ...
      try {
        await updateAnnotation(slideID, annotationDetails.annotationId, "PATHOLOGY", updatedPayload);
  ...
  const handleAnnotationClick = async (selectedAnnotation) => {
    ...
    setSelectedAnnotationId(selectedAnnotation.annotation_id);
  ...
  //     if (response) {
  //       toast.success("Case referred to Senior Pathologist");
  ...
  //     }
  //   } catch (err) {
  //     toast.error("Senior Pathologist not found.");
  //   }
  // };
  ...
  const handleGoBack = () => {
    ...
    setShowForm(false);
    navigate(
      `/pathology-annotation-tool/${selectedSlide.slideID}/case-details`,
      {
            ...
            <section className="annotations-card-header-container">
              {showForm ? (
                <section className="anno-header">
                  ...
                  <p className="heading">Add Annotation</p>
                </section>
              ) : (
                <EditorCardHeader
